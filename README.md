...
| 页面 | 玩法 |
|------|------|
| 开始页 | 点击大圆点进入游戏（呼吸动画，每3秒换色） |
| 页1 · 变色音符 | 点击3个圆点→变色+弹跳+对应音调；点击3次后出现翻页箭头；圆点大小响应式适配小屏 |
| 页2 · 变大变小 | 双指捏合/张开缩放；单指拖拽变形；**单指点击变色+音调**；松手果冻回弹 |
| 页3 · 大爆炸 | 长按圆点1.5秒触发分裂，**多段裂变：1→2→4→8→16**；每个子圆点都可继续长按裂变；短按变色 |
| 页4 · 摇一摇 | 摇动手机→圆点随机重排+音效；倾斜设备→重力感应圆点滚动；iOS需点按钮一次性授权 |
| 页5 · 连一连 | 点击1号点激活，手指划过下一个目标点自动吸附连接；当前目标点有脉冲高亮提示；全连完庆祝+显示翻页 |
| 页6 · 自由画布 | 点击生成圆点+音调；滑动画彩色线条；拖拽移动圆点；长按删除；右上角📷保存截图 |
| 结束页 | 满屏彩色圆点飘落，点击任意位置重新开始 |
